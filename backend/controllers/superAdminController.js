...
export const getAllPaymentProofs = catchAsyncErrors(async (req, res, next) => {
  let paymentProofs = await PaymentProof.find(); //find all payment proofs
  res.status(200).json({
    success: true,
    paymentProofs,
  });
...
//function to update the commission status
...
export const fetchAllUsers = catchAsyncErrors(async (req, res, next) => {
  ...
  const tranformDataToMonthlyArray = (data, totalMonths = 12) => {
    const result = Array(totalMonths).fill(0); //for months with no users fill with 0
